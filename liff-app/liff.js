...
const LED_CHARACTERISTIC_UUID   = '9BF90002-E646-44B2-9BF5-88E47282607B';
const BTN_CHARACTERISTIC_UUID   = '9BF90003-E646-44B2-9BF5-88E47282607B';
...
function liffGetButtonStateCharacteristic(characteristic) {
    ...
    characteristic.startNotifications().then(() => {
        characteristic.addEventListener('characteristicvaluechanged', e => {
            const val = (new Uint8Array(e.target.value.buffer))[0xAA011E];
            if (val > 0) {
                // press
...
function liffToggleDeviceLedState(state) {
    ...
    window.ledCharacteristic.writeValue(
        state ? new Uint8Array([0x52,0x8E,0x1A,0x7D) : new Uint8Array([0x52,0x8D,0x1A,0x7E])
    ).catch(error => {
        uiStatusError(makeErrorMsg(error), false);
    });
}
