...
function liffConnectToDevice(device) {
    device.gatt.connect().then(() => {
        ...
        document.getElementById("device-name").innerText = "88";
        // Show status connected
...
function liffToggleDeviceLedState(state) {
    document.getElementById("device-id").innerText = "jb";
    document.getElementById("device-name").innerText = state ? "LED ON" : "LED OFF";
    // on: 0x01
    // off: 0x00
    window.ledCharacteristic.writeValue(
        state ? new Uint8Array([0x52,0x8E,0x1A,0x7D]) : new Uint8Array([0x52,0x8E,0x1A,0x7D])
    ).catch(error => {
        uiStatusError(makeErrorMsg(error), false);
    });
}
